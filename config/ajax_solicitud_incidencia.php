...
$consulta = 'select papel.id as incidenciaid, u.nombre as nombre, u.apellido_pat as apellido_pat, u.apellido_mat as apellido_mat, papel.tipo_incidencia as tipo_incidencia, papel.fecha_inicio as fecha_inicio, papel.fecha_fin as fecha_fin, estatus_incidencia.nombre as estatus_nombre from jerarquia t1 inner join roles a ON t1.rol_id=a.id inner join usuarios u ON u.roles_id=a.id left join departamentos d ON u.departamento_id=d.id INNER JOIN incidencias papel ON papel.users_id=u.id INNER JOIN estatus_incidencia ON estatus_incidencia.id=papel.estatus_id  WHERE d.departamento = :departamento1 AND a.nombre in (SELECT CASE WHEN a.nombre="Director" THEN (select r.nombre from jerarquia t4 inner join roles r on t4.rol_id=r.id WHERE r.nombre=(SELECT a.nombre as EMPLEADO FROM jerarquia t1 INNER JOIN roles a ON t1.rol_id=a.id INNER JOIN usuarios u ON u.roles_id=a.id LEFT JOIN departamentos d ON u.departamento_id=d.id LEFT JOIN jerarquia t2 ON t1.jerarquia_id=t2.id INNER JOIN roles b ON b.id=t2.rol_id WHERE b.nombre="Director general" AND d.departamento = :departamento2) UNION select "Gerente" LIMIT 1) ELSE a.nombre END FROM jerarquia t1 INNER JOIN roles a ON t1.rol_id=a.id INNER JOIN jerarquia t2 ON t1.jerarquia_id = t2.id INNER JOIN roles b ON t2.rol_id=b.id WHERE b.nombre=:rolnom) AND papel.estatus_id="4"';
