<?php
	...
	if (!empty($estado)) {
		...
		// Consulta para verificar si existen datos en la tabla temporal
		$temporales = $object->_db->prepare("SELECT municipio_id FROM expedientes_temporales WHERE id = :expedienteid");
		$temporales->execute(array(':expedienteid' => $expediente_id));
		$temporalesCount = $temporales->rowCount();

		if ($temporalesCount > 0) {
			$fetchtemporales = $temporales -> fetch(PDO::FETCH_ASSOC);
			if($fetchtemporales['municipio_id'] !== null){
				...
				while ($r = $municipios->fetch(PDO::FETCH_OBJ)) {
					echo "<option value='".$r->Id."'";  if($fetchtemporales['municipio_id'] == $r->Id){ echo 'selected=\"selected\"'; } echo ">";
					echo $r->nombre;
					echo "</option>";
				}
				echo "</select>";
