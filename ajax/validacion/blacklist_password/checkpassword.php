...
if($output === true){
    ...
    if($fetch_password -> password == $sha1_password){
        ...
        $repeated_password = $object ->_db->prepare("SELECT historial_password.password, historial_password.today_date FROM usuarios INNER JOIN reset_password ON reset_password.user_id=usuarios.id INNER JOIN historial_password ON historial_password.user_id=usuarios.id WHERE reset_password.reset_link_token=:token");
        $repeated_password -> execute(array(':token' => $token));
