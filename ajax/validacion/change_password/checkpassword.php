...
if($output === true){
    ...
    $check_password = $object ->_db->prepare("SELECT password from usuarios WHERE id=:useridselect");
    $check_password->execute(array(':useridselect' => $id_user));
    ...
    if($fetch_password -> password == $sha1_password){
        ...
        $repeated_password = $object ->_db->prepare("SELECT historial_password.password, historial_password.today_date FROM historial_password WHERE historial_password.user_id=:userid");
        $repeated_password -> execute(array(':userid' => $id_user));
