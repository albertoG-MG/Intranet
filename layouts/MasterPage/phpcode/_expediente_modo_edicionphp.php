<?php
    ...
    $buscar_papeleria="true";
            ...
            $checktipospapeleria = $object -> _db -> prepare("SELECT * FROM tipo_papeleria WHERE tipo_papeleria.nombre NOT IN('EVALUACION PSICOMETRICA','CONTRATO DEFINITIVO', 'ALTA DE IMSS', 'CONTRATO NOMINA BANCARIA', 'CONTRATO DE PRUEBA', 'CONTRATO INTERNO', 'CONTRATO SUPERVIVENCIA', 'MODIFICACION SALARIAL', 'REGLAMENTO INTERIOR DEL TRABAJO', 'CARTA RESPONSIVA DE EQUIPOS ASIGNADOS', 'BAJA ANTE IMSS', 'EVALUACION PSICOMETRICA', 'CARTA DE SEGUNDO TRABAJO', 'ACTA DE MATRIMONIO', 'IMAGEN DE DATOS BANCARIOS', 'CONTRATO INDETERMINADO', 'CONVENIO DE CONFIDENCIALIDAD','CONTRATO DETERMINADO')");
            $checktipospapeleria->setFetchMode(PDO::FETCH_ASSOC);
            $checktipospapeleria->execute(array(':expedienteid' => $fetch_token_user -> idExpediente));
            $counttipospapeleria = $checktipospapeleria -> rowCount();
            $papeleria = $checktipospapeleria->fetchAll();
