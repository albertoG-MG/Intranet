    ...
    <ul id="catalogos" class="hidden py-2 space-y-2">
        <li>
            ...
            <?php if (Permissions::CheckPermissions($_SESSION["id"], "Acceso a roles") == "true" || Roles::FetchSessionRol($_SESSION["rol"]) == "Superadministrador") { ?>
                <?php if(basename($_SERVER['PHP_SELF']) == 'roles.php' || basename($_SERVER['PHP_SELF']) == 'permisos.php' || basename($_SERVER['PHP_SELF']) == 'crear_rol.php' || basename($_SERVER['PHP_SELF']) == 'editar_rol.php' || basename($_SERVER['PHP_SELF']) == 'permisocategoria.php'){?>
