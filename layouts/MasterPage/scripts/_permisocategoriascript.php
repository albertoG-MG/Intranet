...
document.addEventListener("DOMContentLoaded", function() {
    $("#datatable").DataTable({
        ...
        buttons: [
            {
                text: "<i class='mdi mdi-animation text-white font-semibold text-lg'></i>Crear categorías",
                attr: {
                    'id': 'categorias',
		...
		"processing": true,
        "serverSide": true,
        "sAjaxSource": '../config/serverside_categorias.php',
		"columns": [
            { 
            ...
            errorPlacement: function(error, element) {
                error.insertAfter(element.parent('.group.flex'));
                    ...
                    rules: {
                        crearcategoria: {
                            required: true,
                            remote: "../ajax/validacion/categorias/checkcategoria.php"
                        }
                    },
                    messages: {
                        crearcategoria: {
                            required: 'Por favor, ingresa una categoria',
                            remote: 'Esa categoria ya existe, por favor escriba otro'
                        }
                    },
                    submitHandler: function(form) {
                        check_user_logged().then((response) => {
		                    if(response == "true"){
                                var fd = new FormData();
                                var categoria = $("input[name=crearcategoria]").val();
                                var app = "categorias";
                                var method = "store";
                                    ...
                                    success: function (response) {
                                        ...
                                        if(response == "success"){
                                            Swal.fire({
                                                ...
                                                text: "Se ha creado una categoria exitosamente!",
                                                icon: "success"
                                            }).then(function() {
	 ...
	 $('#datatable').on( 'click', 'tr .Edit', function () {
            ...
            "<div class='modal-content text-center w-full mt-3 sm:mt-0 sm:mt-0 sm:ml-4 sm:text-left'>"+
                "<div class='grid grid-cols-1 mt-5 mx-6 px-3'>"+
                    ...
                    "<div class='group flex'>"+
                        "<div class='w-10 z-10 pl-1 text-center pointer-events-none flex items-center justify-center'><i class='mdi mdi-animation text-gray-400 text-lg'></i></div>"+
                        "<input class='w-full -ml-10 pl-10 py-2 px-3 rounded-lg border-2 border-indigo-600 mt-1 focus:outline-none focus:ring-2 focus:ring-indigo-800 focus:border-transparent' type='text' id='editcategoria' name='editcategoria' value='"+data[1]+"'>"+
                    "</div>"+
                "</div>"+
            "</div>");
        $('.modal-actions').html(
        ...
        $('#Guardar').validate({
			...
			rules: {
				editcategoria: {
					required: true,
					remote: {
						url: "../ajax/validacion/categorias/checkeditcategoria.php",
						type: "post",
						data: {
							"editarid": data[0]
						}
			...
			submitHandler: function(form) {
				check_user_logged().then((response) => {
					if(response == "true"){
						var fd = new FormData();
						var categorias = $("input[name=editcategoria]").val();
						var app = "categorias";
						var method = "edit";
						var id = data[0];
							...
							success: function (response) {
								...
								if(response == "success"){
									Swal.fire({
										...
										text: "Se ha editado una categoría exitosamente!",
										icon: "success"
									}).then(function() {
										table.ajax.reload(null, false);
										});
...
$(document).ready(function() {
    ...
    <?php
    if(basename($_SERVER['PHP_SELF']) == 'permisocategoria.php'){?>
		    ...
		    if(response == "true"){
                if (result.isConfirmed) {
                    Swal.fire({
                        ...
                        $.ajax({
                            url: "../ajax/eliminar/tabla_categorias/eliminarcategoria.php",
                            type: "post",
                            data: fd,
                            processData: false,
                            contentType: false,
