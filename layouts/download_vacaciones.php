<?php
    ...
    if ((Permissions::CheckPermissions($_SESSION["id"], "Acceso a vacaciones") == "false" || Permissions::CheckPermissions($_SESSION["id"], "Exportar reporte de vacaciones") == "false") && Roles::FetchSessionRol($_SESSION["rol"]) != "Superadministrador" && Roles::FetchSessionRol($_SESSION["rol"]) != "Administrador") {
	...
	$fileName = "reporte-vacaciones_" . date('Y-m-d') . ".xlsx"; 
	
	$excelData[] = array('ID', 'NOMBRE', 'PERIODO_SOLICITADO', 'DIAS_SOLICITADOS', 'FECHA_SOLICITUD', 'ESTATUS');

    $vacaciones_query = $object -> _db ->prepare("SELECT solicitud_vacaciones.id, CONCAT(usuarios.nombre, ' ', usuarios.apellido_pat, ' ', usuarios.apellido_mat) as nombre, solicitud_vacaciones.periodo_solicitado, solicitud_vacaciones.dias_solicitados, solicitud_vacaciones.fecha_solicitud, CASE solicitud_vacaciones.estatus WHEN 1 THEN 'Aprobado' WHEN 2 THEN 'Cancelado' WHEN 3 THEN 'Rechazado' ELSE 'Pendiente' END as estatus FROM solicitud_vacaciones INNER JOIN usuarios ON usuarios.id = solicitud_vacaciones.users_id UNION ALL SELECT historial_solicitud_vacaciones.id, CONCAT(usuarios.nombre, ' ', usuarios.apellido_pat, ' ', usuarios.apellido_mat) as nombre, historial_solicitud_vacaciones.periodo_solicitado, historial_solicitud_vacaciones.dias_solicitados, historial_solicitud_vacaciones.fecha_solicitud, CASE historial_solicitud_vacaciones.estatus WHEN 1 THEN 'Aprobado' WHEN 2 THEN 'Cancelado' WHEN 3 THEN 'Rechazado' ELSE 'Pendiente' END as estatus FROM historial_solicitud_vacaciones INNER JOIN usuarios ON usuarios.id = historial_solicitud_vacaciones.users_id");
    $vacaciones_query -> execute();
    $count_vacaciones = $vacaciones_query -> rowCount();
    if($count_vacaciones > 0){ 
        while($row = $vacaciones_query->fetch(PDO::FETCH_ASSOC)){ 
            $lineData = array($row['id'], $row['nombre'], $row['periodo_solicitado'], $row['dias_solicitados'], $row['fecha_solicitud'], $row['estatus']);  
            $excelData[] = $lineData;
        } 
    }else{
        echo "No hay empleados con vacaciones registradas en la base de datos";
        die();
    }
