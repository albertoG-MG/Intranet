<?php
	...
	$excelData[] = array('ID', 'NOMBRE', 'NUM_EMPLEADO', 'DEPARTAMENTO', 'CORREO', 'NIVEL_ESTUDIOS', 'POSEE_CORREO_ADICIONAL', 'CORREO_ADICIONAL', 'PUESTO', 'CALLE', 'NUM_INTERIOR','NUM_EXTERIOR','ESTADO','MUNICIPIO','CODIGO_POSTAL',
    ...
    'CAPACITACION','FECHA_EUNIFORME','CANTIDAD_POLO','TALLA_POLO','CONTACTO_EMERGENCIA_NOMBRE_1','CONTACTO_EMERGENCIA_PARENTESCO_1','CONTACTO_EMERGENCIA_TELEFONO_1','CONTACTO_EMERGENCIA_NOMBRE_2','CONTACTO_EMERGENCIA_PARENTESCO_2',
    'CONTACTO_EMERGENCIA_TELEFONO_2','RESULTADO_ANTIDOPING','TIPO_SANGRE','VACANTE','FAM_DENTRO_EMPRESA','NOMBRE_FAM','BANCO_PERSONAL','CUENTA_PERSONAL','CLABE_PERSONAL','PLASTICO_PERSONAL','BANCO_NOMINA','CUENTA_NOMINA','CLABE_NOMINA',
    'PLASTICO_NOMINA','SITUACION_EMPLEADO','ESTATUS_EMPLEADO','MOTIVO_EMPLEADO','FECHA_EMPLEADO', 'CURRICULUM Y/O SOLICITUD', 'EVALUACION PSICOMETRICA', 'COPIA ACTA DE NACIMIENTO', 'CURP', 'IDENTIFICACION OFICIAL', 'COMPROBANTE DE DOMICILIO',
    'RFC', 'CARTA DE RECOMENDACION LABORAL', 'CARTA DE RECOMENDACION PERSONAL', 'AVISO DE RETENCION CREDITO INFONAVIT', 'CARTA DE SEGUNDO TRABAJO', 'ALTA DE IMSS', 'CONTRATO NOMINA BANCARIA', 'CONTRATO DE PRUEBA', 'CONTRATO DEFINITIVO', 'CONTRATO INTERNO',
    'CONTRATO SUPERVIVENCIA', 'SITUACION FISCAL', 'CARTA RESPONSIVA DE EQUIPOS ASIGNADOS', 'BAJA ANTE IMSS', 'MODIFICACION SALARIAL', 'COMPROBANTE DE ESTUDIOS', 'CARATULA DE DATOS BANCARIOS', 'ACTA DE MATRIMONIO', 'REGLAMENTO INTERIOR DEL TRABAJO');

    $id_array = [];

    $expedientes_query = $object -> _db ->prepare("SELECT expedientes.*, estados.nombre AS estado_nombre, municipios.nombre AS municipios_nombre, CONCAT(usuarios.nombre, ' ', usuarios.apellido_pat, ' ', usuarios.apellido_mat) AS nombre_usuario, usuarios.correo AS correo, departamentos.departamento as depa_nom, estatus_empleado.situacion_del_empleado AS situacion_empleado, estatus_empleado.estatus_del_empleado AS estatus_empleado, estatus_empleado.fecha as fecha_estatus, estatus_empleado.motivo AS motivo FROM expedientes LEFT JOIN estados ON estados.id=expedientes.estado_id LEFT JOIN municipios ON municipios.id=expedientes.municipio_id INNER JOIN estatus_empleado ON estatus_empleado.expedientes_id=expedientes.id INNER JOIN usuarios ON usuarios.id=expedientes.users_id LEFT JOIN departamentos ON departamentos.id=usuarios.departamento_id"); 
    ...
    if($count_expedientes > 0){ 
        while($row = $expedientes_query->fetch(PDO::FETCH_ASSOC)){ 
            $lineData = array($row['id'], $row['nombre_usuario'], $row['num_empleado'], $row['depa_nom'], $row['correo'], $row['estudios'], $row['posee_correo'], $row['correo_adicional'], $row['puesto'], 
            ...
            $row['curp'], $row['nss'], $row['rfc'], $row['tipo_identificacion'], $row['num_identificacion'], $row['capacitacion'], $row['fecha_enuniforme'], $row['cantidad_polo'], $row['talla_polo'], $row['emergencia_nombre']
            , $row['emergencia_parentesco'], $row['emergencia_telefono'], $row['emergencia_nombre2'], $row['emergencia_parentesco2'], $row['emergencia_telefono2'], $row['resultado_antidoping'], $row['tipo_sangre'], $row['vacante']
            , $row['fam_dentro_empresa'], $row['fam_nombre'], $row['banco_personal'], $row['cuenta_personal'], $row['clabe_personal'], $row['plastico_personal'], $row['banco_nomina'], $row['cuenta_nomina'], $row['clabe_nomina'], $row['plastico']
            , $row['situacion_empleado'], $row['estatus_empleado'], $row['motivo'], $row['fecha_estatus']);  
            $excelData[] = $lineData;
            $id_array[] = $row['id'];
        } 
    }else{
